...
```{r}
caminho <- "https://raw.githubusercontent.com/arpanosso/ministerio_saude_alimentos/main/dados/rodada4.txt"
dados <- read.table(caminho,h=TRUE,sep="\t")
glimpse(dados)
```


# Gráfico ao longo do tempo para IPCA_cristal e Açúcar refinado Exportação

```{r}
coeff <- 1
dados %>%   
  ggplot(aes(x=Ano)) +
  geom_line( aes(y=IPCA_cristal, color = "IPCA_cristal")) + 
  geom_line( aes(y=refinadoExp  / coeff, color = "refinado Exportação")) + 
  scale_y_continuous(
    name = "IPCA",
    sec.axis = sec_axis(~.*coeff, name="Açúcar refinado exportação")
  ) +
  scale_colour_manual(values = c("blue","red"))
```

# Gráfico ao longo do tempo para IPCA_cristal e Açúcar bruto Exportação
...
dados %>%   
  ggplot(aes(x=Ano)) +
  geom_line( aes(y=IPCA_cristal, color = "IPCA_cristal")) + 
  geom_line( aes(y=brutoExp  / coeff, color = "bruto exportação")) + 
  scale_y_continuous(
    name = "IPCA",
    sec.axis = sec_axis(~.*coeff, name="Açúcar bruto exportação")
  ) +
  scale_colour_manual(values = c("red", "blue"))
```

# Gráfico ao longo do tempo para IPCA_refinado e Açúcar refinado Exportação

```{r}
coeff <- 1
dados %>%   
  ggplot(aes(x=Ano)) +
  geom_line( aes(y=IPCA_Refinado, color = "IPCA_refinado")) + 
  geom_line( aes(y=refinadoExp  / coeff, color = "refinado Exportação")) + 
  scale_y_continuous(
    name = "IPCA",
    sec.axis = sec_axis(~.*coeff, name="Açúcar refinado exportação")
...
# Gráfico ao longo do tempo para IPCA_refinado e Açúcar bruto Exportação
...
dados %>% 
  ...
  cor()
```
# Análise de regressão linear para açúcar refinado exportação e IPCA_cristal 

```{r}
dados %>% 
  ggplot(aes(x=refinadoExp, y=IPCA_cristal)) +
  geom_point(shape=21, color="black", fill="aquamarine4", size=4)+
  geom_smooth(aes(x=refinadoExp, y=IPCA_cristal),method = "lm",color="red") +
  theme_classic()+
  stat_regline_equation(aes(
  label =  paste(..eq.label.., ..rr.label.., sep = "*plain(\",\")~~")))
```

## Anova para açúcar refinado exportação e IPCA_cristal

A análise de regressão linear foi significativa pelo teste F ao nível de 1% de probabilidadade (p=<0.001), indicando que existe uma relação linear entre o IPCA_cristal e refinadoExp, denotado pelos altos valor de R² (0.89).

```{r}
da<-dados
mod <- lm(IPCA_cristal~refinadoExp, data=da)
summary.lm(mod)
```

# Análise de regressão linear açúcar bruto e IPCA_cristal

```{r}
dados %>% 
  ggplot(aes(x=brutoExp, y=IPCA_cristal)) +
  geom_point(shape=21, color="black", fill="aquamarine4", size=4)+
  geom_smooth(aes(x=brutoExp, y=IPCA_cristal),method = "lm",color="red") +
  theme_classic()+
  stat_regline_equation(aes(
  label =  paste(..eq.label.., ..rr.label.., sep = "*plain(\",\")~~")))
```

## Anova para açúcar bruto exportação e IPCA_cristal

A análise de regressão linear foi significativa pelo teste F ao nível de 1% de probabilidadade (p=<0.001), indicando que existe uma relação linear entre o IPCA_cristal e brutoExp, denotado pelos altos valor de R² (0.90).

```{r}
da<-dados
mod <- lm(IPCA_cristal~brutoExp, data=da)
summary.lm(mod)
```

# Análise de regressão linear para açúcar refinado exportação e IPCA_refinado 

```{r}
dados %>% 
  ggplot(aes(x=refinadoExp, y=IPCA_Refinado)) +
  geom_point(shape=21, color="black", fill="aquamarine4", size=4)+
  geom_smooth(aes(x=refinadoExp, y=IPCA_Refinado),method = "lm",color="red") +
  theme_classic()+
  stat_regline_equation(aes(
  label =  paste(..eq.label.., ..rr.label.., sep = "*plain(\",\")~~")))
```

## Anova para açúcar refinado exportação e IPCA_refinado

A análise de regressão linear foi significativa pelo teste F ao nível de 1% de probabilidadade (p=<0.001), indicando que existe uma relação linear entre o IPCA_refinado e refinadoExp, denotado pelos altos valor de R² (0.92).

```{r}
da<-dados
mod <- lm(IPCA_Refinado~refinadoExp, data=da)
summary.lm(mod)
```

# Análise de regressão linear açúcar bruto e IPCA_refinado

```{r}
dados %>% 
  ggplot(aes(x=brutoExp, y=IPCA_Refinado)) +
  geom_point(shape=21, color="black", fill="aquamarine4", size=4)+
  geom_smooth(aes(x=brutoExp, y=IPCA_Refinado),method = "lm",color="red") +
  theme_classic()+
  stat_regline_equation(aes(
  label =  paste(..eq.label.., ..rr.label.., sep = "*plain(\",\")~~")))
```
...
A análise de regressão linear foi significativa pelo teste F ao nível de 1% de probabilidadade (p=<0.001), indicando que existe uma relação linear entre o IPCA_refinado e brutoExp, denotado pelos altos valor de R² (0.92).

```{r}
da<-dados
mod <- lm(IPCA_Refinado~brutoExp, data=da)
summary.lm(mod)
```